...
export default function ImportMapping({ filePath, headers, category }: Props) {
    ...
    transform((data) => ({
        ...
        mapping: {
            ...
            sort_order: data.mapping.sort_order === 'none' ? '' : data.mapping.sort_order,
        }
    }));

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('master-data.import.process'));
    };
    ...
    const requiredFields = [
        { key: 'key', label: 'Key / ID (Verplicht)' },
        { key: 'label', label: 'Naam / Label (Verplicht)' },
    ];

    const optionalFields = [
        { key: 'description', label: 'Omschrijving (Optioneel)' },
        { key: 'sort_order', label: 'Sorteervolgorde (Optioneel, getal)' },
    ];
    ...
    return (
        <AppLayout breadcrumbs={[
            ...
            <div className="flex h-full flex-col gap-4 p-4 max-w-4xl mx-auto w-full">
                <Card>
                    <CardHeader>
                        <CardTitle>Koppel Data Kolommen (Stamgegevens)</CardTitle>
                        <CardDescription>
